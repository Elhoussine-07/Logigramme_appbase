...
def get_last_value(cursor, var_id):
    ...
    cursor.execute(
        "SELECT TOP 1 val_valide FROM his_valeur WHERE id_variable = ? AND id_qualfication=0 ORDER BY date_acquisition DESC",
        var_id
    )
...
def insert_result(cursor, var_id, valeur):
    cursor.execute("""
        INSERT INTO his_valeur (
            id_variable, date_acquisition,  id_qualification, val_brute, val_valide
        )
        VALUES (?, GETDATE(), 1, ?,?)
